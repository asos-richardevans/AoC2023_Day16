...
var energisedCells = new List<(int, int)> { (0, 0) };
var beams = new List<(int, int, int, Direction)> { (0, 0, 1, Direction.Right) };
...
while (beams.Any())
{
    for (var i = 0; i < beams.Count; i++)
    {
        ...
        if (currentCellValue == '.' ||
            (currentCellValue == '-' && (currentDirection == Direction.Left || currentDirection == Direction.Right)) ||
...
Console.WriteLine(energisedCells.Count);
